...
o={
  ...
  ob3:new T.Object3D,
  sty:'canvas{left:0;top:0}canvas,#opsMenu{position:fixed}#opsMenu{background:rgba(0,0,0,.5);border-radius:4px}#opsMenu>div>*{background:rgba(0,0,0,.5);border-radius:4px}#opsMenu>div>:first-child,#opsMenu>div>:last-child{display:block;margin-left:.7em}#opsMenu>:first-child{margin:0}',
  ...
  mn:function(i,ch){
    u.ap(o.mn=u.new('div',{id:'opsMenu'}),u.b)
    o.mn.rt=function(xy,p){
      dr.o&&dr.o.id!=6&&(
        dr.o.rotation[xy]-=o.π/2)
      0}
    o.mn.fn=[
      ...
      u.f('dr.o&&dr.o.id!=6&&(dr.o.rotation.x+=o.π/2)'),//raise or stand up
      u.f('dr.o&&dr.o.id!=6&&(dr.o.rotation.y+=o.π/2)'),//rotate to the left
      u.f('dr.o&&dr.o.id!=6&&(dr.o.rotation.y-=o.π/2)'),//rotate to the right
      u.f('dr.o&&dr.o.id!=6&&(dr.o.rotation.x-=o.π/2)')]//lie
    //'🌄✖⤴↩↪⤵'

    o.mn.innerHTML='<button>🌄</button><button>✖</button><div><button>⤴</button><button>↩</button><button>↪</button><button>⤵'
    ch=o.mn.children
    for(i in ch){
      if(isNaN(i))return
      else ch[i].idx=i,ch[i].onclick=u.f('o.mn.fn[this.idx]()')}
    ch[2].onmousedown=u.f('dr.o&&dr.o.rotateY(o.π/20)')
    0},
...
/*let it be exactly after left container
  ...
  init:function(){var g,m,φ,M,px,py,i=0,clck,msh
    ...
    u.sty(o.sty+='#fs,canvas,button{-'+o.pfx+'-user-select:none')
