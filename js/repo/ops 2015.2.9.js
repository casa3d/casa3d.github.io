...
o={
  ...
  ob3:new T.Object3D,
  sty:'canvas{left:0}canvas{top:0}canvas,#mn,#dw,#opsMenu{position:fixed}#opsMenu{background:rgba(0,0,0,.5);border-radius:4px}#opsMenu>div>*{background:rgba(0,0,0,.5);border-radius:4px}#opsMenu>div>:first-child,#opsMenu>div>:last-child{display:block;margin-left:.7em}#opsMenu>:first-child{margin:0}',
  ...
  Δ:[],//its length will be 6;I don't think user´d put into scene more this Quantity of digits (of same texture type)
  clear:function(clck,msg,tm){
    setTimeout(function(){
      clearInterval(clck)
      //console.clear()
      u.c(msg+"-renderer clock has been cleared")},tm)},
  ...
  ad:function(tx,blob){//texture, blob
    ...
    setTimeout(function(v,i,g,o){
      ...
      cp.y+=.1
      cp.z+=.1},10)},
  add:function(tx,blob,sz,up){var tn,ptx,g,m,M,idx,lg,ob
    ...
    ob?(
      ...
      m.map=φ=blob?(
        ...
        T.ImageUtils.loadTexture(o.tx))
        :(o.tn[tx]={id:o.tns.children.length,x:1},
        o.o.material.map),
      up&&dr.o&&(
        o.mn.up=0,
        dr.o.material.map=T.ImageUtils.loadTexture(o.tx)),
  ...
  cc:function(){/*var c=t.s.__lights[0].color//change color lighting intensity
    ...
    u.c(c.r=c.g=c.b=this.value/100)},//and finally match them all
...
/*let it be exactly after left container
  ...
  o:{id:''},
  kbd:u.f('o.kb&&(!o.kb.position.z&&(o.kb.position.z=-1))'),
  ...
  rc:function(e){//10% & position of mouse in Y
    o.V3.x=(e.clientX/innerWidth)*2-1,o.V3.y=-((o.y=e.clientY)/innerHeight)*2+1
    ...
    o.app&&(T=o.tns,x=T.position.x,
      ((l=o.x&&x>T.right)||!o.x&&x<T.left)&&(
          T.position.x-=l?
            ...
            o.R()))},
  at:u.f(//addMenu&thumbnailsMenu proceed only if it comes from thumbnails or rll's children
  ...
  init:function(){var g,mt,φ,M,px,py,i=0,clck,msh,D
    ...
    o.ops[2]=o.how
    o.ops[4]=o.op[3]=o.shwm
    o.ops[10]=o.ops[11]=o.cc
    o.ops[14]=o.op2[0]=o.exp
    ...
    u.sty(o.sty+='#fs,canvas,button,#mn{-'+o.pfx+'-user-select:none')
...
/*    v=[[-.3,3.5], [.3,3.5],//Original matrix
    ...
    ops=mn.children[1]
    for(i in o.e)ops.innerHTML+='<'+bt+o.e[i],msh(0),msh(1),i++==4&&(M.il=1)//increase light
    edt.insertBefore(u.at(o.r.domElement,{id:'ops'}),h.tls)//adding all to body as canvas
    o.mn()
    //地点: location (Dìdiǎn)
    u.ap(D=u.new('div',{id:'地点',innerHTML:'<div><button></button><button>🔘</button></div><div><input><input></div><div><input><input></div><div><input><input>'}),mn)
    u.ap(u.new('div',{innerHTML:'<button id=cmr>📹</button><input type=number min=-10 max=10>'}),D)
    mn.innerHTML+='<div><'+bt+'💾</'+bt+'<'+bt+'🔒</'+bt+'<'+bt+'ⓘ'
    u.ap(u.new('button',{id:'dw',innerHTML:''}),h.tls)
    ...
    o.in=u.new(
      ...
      accept:'image/*',
      multiple:1,
      onchange:u.f('o.rb(this.files)')})
    clck=setInterval(o.R,500)
    o.clear(clck,'ops',6e3)
    o.re=RegExp(o.left.id+'|'+o.menu.children[0].id+'|'+o.menu.children[2].id+'|'+o.right.id)
    u.sty("#mn{background:rgba(0,0,0,.6);font-size:1.5em;width:18%}#mn>:nth-child(odd){background:linear-gradient(rgba(155,251,228,.8),rgba(5,80,109,.5));color:#FD0}#mn>:nth-child(even){background:linear-gradient(rgba(15,89,122,.5),rgba(79,159,140,.8))}#lit{width:60%}#mn button{color:rgba(255,255,255,.8);margin:-.01em}#mn button:hover{background:radial-gradient(rgba(155,251,228,.8),rgba(5,80,109,.5));text-shadow:#000 -15px 10px 7px;color:#FFF}#地点>:nth-child(1)>*{background:rgba(83,111,117,.5);box-shadow:#000 0 -2px 10px;width:50%}#地点>:nth-child(2):before{content:'X '}#地点>:nth-child(3):before{content:'Y '}#地点>:nth-child(4):before{content:'Z '}#地点 input{width:40%;font-size:1.25em}#dw{paddin:0}#dw:hover{rgba(0,178,255,.5)}#mn,#dw{left:0;top:10%")
    setTimeout(function(){
      //u.a=!u.a
      lit.onchange=o.cc
      var mc=mn.children[1].children
      for(i in o.op)mc[i].onclick=o.op[i]
      cmr.onclick=function(){
        this.innerHTML=this.innerHTML=='🎥'?
          '📹'
          :'🎥'}
      cmr.nextSibling.onchange=function(){var v;if(v=this.value)u.c(m.i=v)}

      dc=地点.nextSibling.children//💾 🔒 ⓘ
      for(i in o.op2)dc[i].onclick=o.op2[i]
      u.c(D)
      m[0]=地点.children[1].firstChild
      m[1]=地点.children[2].firstChild
      m[2]=地点.children[3].firstChild
      dw.onclick=function(){
        this.innerHTML=this.innerHTML==''?'':''
        mn.style.left=mn.style.left?'':'-34%'}},1e3)}}
o.init()
o.ae(ops,
  {click:function(e){var i=o.o.id,sf
    i&&(
      o.ops[i]?
        o.ops[i]()
    ...
    //left kbrd add right
    !o.st.pnl.position.y&&!o.re.test(i)&&o.shwm()
    i!=o.left.id&&i!=o.right.id&&u.clk(o.zip)
  ...
  contextmenu:function(e){
    ...
    u.clk(function(){
      u.sc(
        o.mn,
        dr.o&&dr.o.id!=6?(
          o.mn.setAttribute('style','left:'+e.clientX+'px;top:'+e.clientY+'px'),
          '')
          :'no')})},
      ...
      o.o.id!=o.rst.object.id&&(
        o.o.id&&(
          o.at()?(//set ps.z & opacity only if it comes from thumbnails or rll's children
            ...
            :o.mc&&(//let's restore its OST color, (I mean Original)
