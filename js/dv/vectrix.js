v={
 rgx:[//set of RegExp that they'll help me to evaluate and decode vertices
  ...
  /(\S)\|([^\s;])/,
  /(.?);(.?)/],
 abx:[//these are going to be used while evaluating and decoding
  ' rR ',
  '|0',
  '|0',
  '0',
 ...
 psx:'|!'.s(),
 mpf:'',
 pC:F('n','return v.pf(n.toFixed(5))'),
 pF:F('n','return v.pf(n)*(v.mpf?Math.pow(10,v.mpf):1)'),
 ...
 v:function(a,m){var i,A
  v.mpf=m||2
  for(i=0;i<v.abx.length;i++)while(ab=v.rgx[i].exec(a))a=a.replace(ab[0],ab[1]+(v.pf(i)==0?' '+/\S([rR¡])[^\s\/\*\+-]/.exec(a)[1]+' ':v.abx[i])+ab[2])
  v.a=a.S(' ')//finishing vector
  for(i=0;i<v.a.length;i++){
   v.a[i]=v.a[i].s(',')//separating vector's values
   A=B=v.a[i],l=[,0]
